...
void Lcd_CreateCustomChar(Lcd_HandleTypeDef * lcd, uint8_t location, uint8_t *charmap)
{
    uint8_t i;

    // Set CGRAM address
    location &= 0x07;  // Limit location to 8 characters
...
void Lcd_Init(Lcd_HandleTypeDef * lcd)
{
    ...
    for (uint8_t i = 0; i < 8; i++)
        {
            Lcd_CreateCustomChar(lcd, i, custom_chars[i]);
        }
}
